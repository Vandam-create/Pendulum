...
int main() {
    ...
    sf::RenderWindow window(sf::VideoMode({1000, 800}), "PBD");
...
//////////////////////////////
    ...
    std::vector<sf::CircleShape> gr_particles;
    ...
    while (window.isOpen()) {
        ...
        while (auto event = window.pollEvent()) {
            ...
            if(!isRunning){
                ...
                if (auto* mouse_ev = event->getIf<sf::Event::MouseButtonPressed>()) {
                    
                    sf::Vector2f mousePos(mouse_ev->position);
                    
                    const auto& circles = pendulums.get_circles();
                    for(size_t i = 0; i < circles.size(); ++i){
                        if (circles[i].getGlobalBounds().contains(mousePos)) {
                            isDraging = true;
                            index_for_constraint = i;
                        }
                    }

                }

                if(auto* mouse_ev = event->getIf<sf::Event::MouseMoved>()) {
                    if(isDraging){
